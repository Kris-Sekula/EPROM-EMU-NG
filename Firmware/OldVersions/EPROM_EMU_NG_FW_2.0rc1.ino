...
void setup() {
  ...
  if(HW_ver[0] != 'v') {
    ...
    //
    EEPROM.put(ver_cfg, "v1.6"); //Set this to whatever you have on the PCB, if you did the HW mod as per Errata, call it v1.6f or similar   <--------- README                                  
...
void loop() {
  ...
  if (digitalRead(LD_BTN) == LOW) {
    ...
    EEPROM.write(auto_cfg,0);
    autoLoad = false;
    Serial.println(F("Auto load disabled by pushbutton"));
    delay(5000);
    }
  }
  ...
  if (Serial.available() > 0) {
    ...
    if ( ch == 0x0A /*LF*/) {
      ...
      if ( rxFrame == ":dml" ) {
        digitalWrite( WE, LOW);       // brings data bus of SRAM to high-z
        digitalWrite( EN_RST, HIGH ); // reset target, disable 541 buffers, enable shift registers
        digitalWrite( LED_G, HIGH );  // RUN LED off
        printabout();
      }
      //
      // start emulation
