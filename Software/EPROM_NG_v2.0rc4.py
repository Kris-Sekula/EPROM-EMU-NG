...
# ver 2.0rc4 - Mar 2025     - move to using FreeSimpleGUI, only load gui library if needed, show friendly import errors 	
...
def importGUI():
	try:
		...
		print(f"""Looks like you are missing the graphical interface library
		You should run the following command to install it:
...
if len(sys.argv) >= 2:
	...
	form = sg.FlexForm(f'Eprom emulator Uploader version: {version}')
	...
	layout = [  [sg.Text('Eprom Type'), sg.InputCombo(('2716', '2732','2764', '27128','27256', '27512'),sg.user_settings_get_entry('mem',''), key='mem', size=(10, 6))],		
				[sg.Text('COM port   '), sg.InputCombo(get_com_ports(),sg.user_settings_get_entry('port',''),key='port',size=(10, 1)),sg.Button("Refresh Ports", key='refresh_ports')],														
